...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case 'FETCH_SUCCESS':
      return { ...state, product: action.payload, loading: false }; //data in action is payload
    case 'FETCH_FAIL':
...
function ProductScreen() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const result = await axios.get(`/api/products/slug/${slug}`); //send ajax request to address and put result in 'result'
